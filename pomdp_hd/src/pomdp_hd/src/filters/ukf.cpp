        ...
        Eigen::VectorXd& x_bar, Eigen::MatrixXd& P_bar)
{
    ...
    P_aug_hat.block(n_x, n_x, n_v, n_v) = dynamics[activeIdx]->V_mat();
    ...
    for(unsigned int i=0; i<sigma_points.size(); i++)
    {
        ...
        // dynamics[activeIdx]->propagateState(nx_sigma_pt, u, x_i, s_to_LA);
        dynamics[activeIdx]->propagateStateUseTransform(nx_sigma_pt, u, x_i, s_to_LA);

        x_i += v_i;
        new_sigma_pt << x_i; //
        predicted_sigma_pts.push_back(new_sigma_pt);
        // std::cout << "sigma_pt : " << new_sigma_pt.transpose() << std::endl;
    }
        ...
        Eigen::VectorXd& x_new, Eigen::MatrixXd& P_new)
{
    ...
    P_aug_bar.block(n_x, n_x, n_z, n_z) = dynamics[activeIdx]->W_mat();
