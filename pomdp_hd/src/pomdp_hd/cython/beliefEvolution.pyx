...
cdef class BeliefEvolution:
    ...
    def setIntergationStepSize(self, double step_size):
        return self.cobj.setIntergationStepSize(step_size)

    def getMatrices(self, int idx, np.ndarray mu, np.ndarray u, np.ndarray A, np.ndarray B, np.ndarray C, np.ndarray V, np.ndarray W):
        return self.cobj.getMatrices(int(idx), Map[VectorXd](mu), Map[VectorXd](u), Map[MatrixXd](A), Map[MatrixXd](B), Map[MatrixXd](C), Map[MatrixXd](V), Map[MatrixXd](W))

    def forwardKinematicsMapped(self, int idx, np.ndarray q, np.ndarray pose):
        return self.cobj.forwardKinematicsMapped(int(idx), Map[VectorXd](q), Map[VectorXd](pose))

    def inverseKinematicsMapped(self, int idx, np.ndarray q, np.ndarray pose):
        return self.cobj.inverseKinematicsMapped(int(idx), Map[VectorXd](q), Map[VectorXd](pose))
    ...
    def setReferenceFrameMapped(self, np.ndarray x):
        return self.cobj.setReferenceFrameMapped(Map[VectorXd](x))
