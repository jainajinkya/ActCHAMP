...
class Rigid_model(object):
    """docstring for Rotational_model"""
    def __init__(self):
        super(Rigid_model, self).__init__()
    ...
    def generate_actions(self):
        arr = np.random.uniform(-100.,100., (self.Data_len, 3))
    ...
    def visualize(self, data, do_plot=False, do_animate=False):
        ...
        if do_plot:
            ...
            ax.set_title('Simulator for Rigid_model')
        ...
        if do_animate:
            ...
            ax2.set_title('Simulator for Rigid_model')
    ...
    def save_data(self, data, filename="Rigid_model.txt"):
...
def main():
    ...
    rigid_model.print_attributes(True)

    pos = deepcopy(rigid_model.origin)
    pos_set = [pos.T]
    action_set = deepcopy(rigid_model.generate_actions())
    # print "Action_set = ", action_set

    for action in action_set:
        pos = deepcopy(rigid_model.simulate_one_step(pos, action))
    ...
    rigid_model.visualize([pos_set, action_set], do_plot, do_animate)
    
    # Save data
    filename = "../data/sim_data/Rigid_data.txt"
    rigid_model.save_data([pos_set, action_set], filename)
