...
class KinovaJogger(object):
    ...
    LEADER_JNT_NAMES = ['right_joint_'+str(i+1) for i in range(7)]
    FOLLOWER_JNT_NAMES = ['left_joint_'+str(i+1) for i in range(7)]
    ...
    def set_target(self, msg):
        ...
        with self.t_lock:
            self.target_pos = self._simplify_angles(np.array([joint_state[j] for j in self.LEADER_JNT_NAMES]))
            self.target_vel = self._simplify_angles(np.array([joint_velocity[j] for j in self.LEADER_JNT_NAMES]))

    def set_current(self, msg):
        joint_state = self._joint_state_dict(msg.name, msg.position)
        joint_velocity = self._joint_state_dict(msg.name, msg.velocity)
        with self.c_lock:
            self.current_pos = self._simplify_angles(np.array([joint_state[j] for j in self.FOLLOWER_JNT_NAMES]))
            self.current_vel = self._simplify_angles(np.array([joint_velocity[j] for j in self.FOLLOWER_JNT_NAMES]))
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Arm Shamdowing for Kinova Arms on Bimanual Robot')
    parser.add_argument('--homing', type=int, default=1, help='Should or should not initiate at home pose. Any non-zero value result in homing.')
    args = parser.parse_args()

    ## Homing
    right_arm = ArmMoveIt(planning_frame="linear_actuator_link",
                          _arm_name="right")
    left_arm = ArmMoveIt(planning_frame="linear_actuator_link",
                         _arm_name="left")
    
    home_position = [4.4617, 1.646, 0.3059, 1.894, 3.029, 1.625, 5.451]
    
    if args.homing:
        right_arm.move_to_joint_pose(home_position)
        left_arm.move_to_joint_pose(home_position)
        rospy.sleep(2)

    ## Shamdowing
    jogger = KinovaJogger(joint_target_topic='/vector/right_arm/joint_states',                  joint_state_topic='/vector/left_arm/joint_states', 
                          joint_vel_topic='/left_arm_driver/in/joint_velocity')
