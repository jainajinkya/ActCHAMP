...
class trajectoryOptimization:
    ...
    def createPlan(self, muInit, covInit, wtsInit, tFinal):
        ...
        X0[self.nState*self.nSegments:self.nState*self.nSegments + self.len_s] = copy.copy(covInit[self.id1])
        ...
        bnds = [(-5., 5.) for i in range(len(X0[:self.nState*self.nSegments]))] + [(0., 10.) for i in range(len(X0[self.nState*self.nSegments:(self.nState+self.len_s)*self.nSegments]))] + [(-1.,1.) for i in range(len(X0[(self.nState+self.len_s)*self.nSegments:(self.nState+self.len_s + self.nInput)*self.nSegments]))] + [(0., 1.) for i in range(len(X0[(self.nState+self.len_s + self.nInput)*self.nSegments:]))]
