...
class pomdp_hd:
    def __init__(self):
        ...
        self.domain = [(-500.0, 500.0)]*self.nState
        ...
        self.allowed_ds_transitions = [[0,1], [1, 0], [2,3], [3,2], [3,1], [0,4], [3,4]]

    def setTrajectoryOptimization(self):
        # Cost Matrices
        Q = 0.05*np.eye(self.nState)
        R = 0.05*np.eye(self.nInput)
        Q_f = 0.5*np.eye(self.nState)
    ...
    def find_set_of_ds_to_cs_goals(self):
        if self.do_global_optimization:
            ...
            self.set_of_ds_goals.append(np.array([100., -10., 100., 10.]))
            self.set_of_ds_goals.append(np.array([120., -10., 75., 10.]))
            self.set_of_ds_goals.append(np.array([120., -10., 120., 314.]))
            self.set_of_ds_goals.append(np.array([120., -10., 95., 314.]))
    ...
    def generateDetailedCSPlan(self, input_plan):
        ...
        for i in range(self.nSegments-1):
            for t in range(1, self.opt.delta+1):
                ...
                ds_new = self.opt.sysDynamics.beliefUpdatePlanning(mu, cov, u_in[:,i], mu_new, cov, w_new, ds_old)
                ...
                u_plan[:,i*self.opt.delta+t-1] = copy.copy(u_in[:,i])

                mu = copy.copy(mu_new)
                
                # if self.do_verbose:
                print "\n Planned Step", i*self.opt.delta+t
                print "Applied Control, u_plan = ", np.round(u_plan[:,i*self.opt.delta+t-1], 2)
