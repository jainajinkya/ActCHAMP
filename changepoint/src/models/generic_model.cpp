...
#include "changepoint/articulation_models/generic_model.h"
#include "changepoint/articulation_models/utils.h"
#include "changepoint/articulation_models/factory.h"
...
//	optimizer_iterations = 0;
	...
	// wt_on_complexity = 1.0;
	wt_on_complexity = 0.0;
...
//		sum_loglikelihood += model.track.channels[channelInlierLogLikelihood].values[i];
	...
	// bic = -2*(loglikelihood ) + complexity * log( n );
	
	//AJ//
	bic = -2*(loglikelihood ) +  wt_on_complexity*complexity * log( n );
	//AJ//
	...
	writeParamsToModel();
	// cout << "Average Error in Position: " << avg_error_position << endl;
	// cout << "Average Error in Orientation: " << avg_error_orientation << endl;
...
double GenericModel::getInlierLogLikelihood( size_t index ) {
	...
	// cout << "At time index " << index << ", Errors \tPosition: " << err_position << "\t Orientation: " << err_orientation << endl;

	double loglikelihood =
			- log(2*M_PI * sigma_position*sigma_orientation)
			- 0.5*(	( SQR(err_position) / (SQR(sigma_position)) ) +
...
// Additional functions for Active CHAMP
geometry_msgs::Pose GenericModel::predictPoseFromAction(size_t index){

	V_Configuration q_cur;
	geometry_msgs::Pose delta_t;

	geometry_msgs::Pose &cur_pose = model.track.pose[index]; 
	q_cur = predictConfiguration(cur_pose);

	if(index <= 1 || q_cur.data() == NULL) 
	{
		/* To avoid accessing random memory data prior to the starting time
		 q_cur.data() ==  NULL corresponds to the rigid body model. NUll data on configuration is handeled by the predictPose function, as it returns the same value as that of input pose */

		delta_t = predictPose(q_cur);
		// cout << "Returning using If condition, q_cur.data: " << q_cur.data() << endl;
		return delta_t;
	}
	else{

		V_Configuration q_prev, dummy(q_cur.size());
		M_CartesianJacobian prev_Jac; //, cur_Jac;
		Eigen::MatrixXd prev_Jac_inv;
		Eigen::Vector3d act, v_t;
		geometry_msgs::Pose &prev_pose = model.track.pose[index-1]; 

		act(0) = model.track.actions[index-1].x;
		act(1) = model.track.actions[index-1].y;
		act(2) = model.track.actions[index-1].z;

		q_prev = predictConfiguration(prev_pose);
		prev_Jac = predictJacobian(q_prev);
		prev_Jac_inv = prev_Jac.transpose()/SQR(prev_Jac.norm()); // Inverse of Jacobian vector. Assuming DOF to be 1 (As are cases here) and a real-valued vector

		v_t = prev_Jac/prev_Jac.norm();	

		// dummy << prev_Jac_inv*(act.dot(v_t)*v_t);
		dummy << prev_Jac_inv*act;

		delta_t = predictPose(q_prev + dummy);

		// cout << "For Time Index: " << index << "\tDot product: " << act.dot(v_t) << endl;
		// cout << "Action: " << act.transpose() << "\t v_t: " << v_t.transpose() << endl;
		// cout << "Error in position current, y: " << delta_t.position.y - cur_pose.position.y  << endl;
		// cout << "Error in position, previous y: " << delta_t.position.y - prev_pose.position.y << endl;
		// cout << "Error in position, prev - cur y: " << cur_pose.position.y - prev_pose.position.y << "\n" << endl;

		// cout << "Predicted Pose: "<< delta_t << "\nActual Pose :" << cur_pose << "\n"<< endl;
		return delta_t;
	}
}


double GenericModel::getInlierLogLikelihoodFromAction( size_t index) {
	// printf("Calculating Inlier Log likelihood from actions\n");
	geometry_msgs::Pose &pose_obs = model.track.pose[index];
	geometry_msgs::Pose pose_estimated = predictPoseFromAction(index);
	model.track.pose_projected[index] = pose_estimated;
...
double GenericModel::getLogLikelihoodForPoseIndexFromAction(size_t index) {
//	double gamma_mixing = 0.1;
//	return (1-gamma_mixing)*getInlierLogLikelihood(pose_obs) + gamma_mixing * getOutlierLogLikelihood();
	// double inlierLikelihood = getInlierLogLikelihood(index);
	double inlierLikelihood = getInlierLogLikelihoodFromAction(index);
	double outlierLikelihood = getOutlierLogLikelihood();
...
double GenericModel::getLogLikelihoodFromAction(bool estimate_outlier_ratio) {
	model.track.pose_projected.resize(model.track.pose.size());
	...
	if(estimate_outlier_ratio) {
		...
		do{
			sum_likelihood = 0;
			for(size_t i=0;i<n;i++) {
				sum_likelihood += getLogLikelihoodForPoseIndexFromAction(i);
			}
...
// -- evaluate model
bool GenericModel::evaluateModelFromAction() {
	// printf("Evaluating Using Model from Action\n");
	...
	// let getLogLikelihood() do the projection
	loglikelihood = getLogLikelihoodFromAction(false);
	// evaluate some extra statistics
