...
def main():
    ...
    with rosbag.Bag(out_bag, 'w') as outbag:
        for pose, act in zip(pose_data, action_data):
            t = rospy.Time(i)
            outbag.write("changepoint_detection/processed_pose_data", DataPoint(pose.tolist()), t)
            outbag.write("changepoint_detection/processed_action_data", DataPoint(act.tolist()), t)
            i += 1
